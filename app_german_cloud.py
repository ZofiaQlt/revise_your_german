...
def display_buttons():
    ...
    with col1:
        if st.button("Réviser de français vers allemand", key="french_to_german", help="Cliquez pour réviser de français vers allemand"):
    ...
    with col2:
        if st.button("Réviser d'allemand vers français", key="german_to_french", help="Cliquez pour réviser d'allemand vers français"):
            st.session_state.revision_direction = 'german_to_french'
            st.session_state.current_word = get_weighted_word(st.session_state.word_scores)
    with col3:
        if st.button("Révision mixte", key="mixed", help="Cliquez pour une révision mixte"):
...
if __name__ == "__main__":
    ...
    if not st.session_state.get('start', False):
        if st.button("Commencer la révision"):
            ...
            st.session_state.current_word = get_weighted_word(st.session_state.word_scores)
            # On ne change pas le répertoire de révision ici
    else:
