...
def display_buttons():
    """Affiche les boutons de sélection de révision."""
    col1, col2, col3 = st.columns(3)
    with col1:
        if st.button("Réviser de français vers allemand"):
    ...
    with col2:
        if st.button("Réviser d'allemand vers français"):
            st.session_state.revision_direction = 'german_to_french'
            st.session_state.current_word = get_weighted_word(st.session_state.word_scores)
            st.experimental_rerun()  # Recharge la page pour commencer la révision
    with col3:
        if st.button("Révision mixte"):
